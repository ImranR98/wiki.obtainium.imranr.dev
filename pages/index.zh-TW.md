...
您可以在 [apps.obtainium.imranr.dev](https://apps.obtainium.imranr.dev) 上找到基於眾包的應用程式設定。
...
!!! info "驗證資訊"
    ...
    - [PGP 公鑰](https://keyserver.ubuntu.com/pks/lookup?search=contact%40imranr.dev&fingerprint=on&op=index)（驗證 APK 雜湊值）

## 應用程式內連結 {#in-app-links}

您可以在 Obtainium 的設定頁面底部找到本 Wiki、原始碼和眾包應用程式設定的連結。
